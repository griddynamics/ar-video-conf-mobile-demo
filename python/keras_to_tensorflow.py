...
def load_model(input_model_path, input_json_path=None, input_yaml_path=None):
    ...
    try:
#        model = keras.models.load_model(input_model_path, custom_objects=None)
        model = get_model()
        return model
...
#        sess = tf.compat.v1.keras.backend.get_session()
    ...
    if FLAGS.quantize:
        ...
        #logging.info('Graph', sess.graph.as_graph_def())
        #logging.info('Graph!!!!!!!!!', converted_output_node_names)
        #logging.info('Graph!!!!!!!!!', orig_output_node_names)        
        constant_graph = graph_util.convert_variables_to_constants(
            sess,
